...
class Perceptron(object):
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for currentWeight in range(len(self.weights)):
        #this is basically like exercising, you change your weight to get good,
            weightLossProgram = inActs[currentWeight] * alpha * delta
            self.weights[currentWeight] += inActs[currentWeight] * alpha * delta
            totalModification += abs(weightLossProgram)
        inActs.pop(0) 
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for layer in layers:
            #multiply each input by weights in each node, do activation, then add that to ouput list.
            layerActivations = []
            for perceptron in layer:
                activation = perceptron.sigmoidActivation(output[-1])
                layerActivations.append(activation)
            #get each perceptron sigmoid activation
            output.append(layerActivations)
        return output
        ...
        for example in examples:#for each example
            ...
            for numLayer in xrange(0,self.numLayers):
                ...
                for numNeuron in xrange(len(layer)):
                    weightMod = layer[
                            numNeuron].updateWeights(allLayerOutput[numLayer],
                            alpha, deltas[numLayer][numNeuron])
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for theInput, output in examplesTest:
        nnetOut = nnet.feedForward(theInput)[-1]
        nnetOutRound = [round(i) for i in nnetOut]
        if nnetOutRound == output:
            testCorrect += 1
    testError = len(examplesTest) - testCorrect
