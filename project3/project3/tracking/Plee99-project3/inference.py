    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            if totalweight == 0:
                ...
                for key in weight:
                    val = weight[key]
                    num = int(val * self.numParticles)
                    for i in range(num):
                        self.particleList.append(key)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = []
        someList = random.shuffle(list(itertools.product(self.legalPositions, repeat=self.numGhosts)))
        for i in range(self.numParticles):
            self.particles[i] = someList[i % len(someList)]
