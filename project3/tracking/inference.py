...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if (noisyDistance is None):
            #get jail position and set to true.
            jail = self.getJailPosition()
            allPossible[jail] = 1.0
            #set the area around it to 0.
            for a in self.legalPositions:
                allPossible[a] = 0
        else:
            for a in self.legalPositions:  #current position is a
                trueDistance = util.manhattanDistance(a, pacmanPosition)
                b = trueDistance  #b is true distance
                #P(A|B) = P(B|A) * P(A)
                allPossible[a] = emissionModel[b]*self.beliefs[a]
                #normalize all possible and set that as the beliefs
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        #iterate through each position it could be at.
        allPossible = util.Counter()
        for legalPosition in self.legalPositions:
            possibleGhostPosition = self.setGhostPosition(gameState, legalPosition)
            newPosDist = self.getPositionDistribution(possibleGhostPosition)
            for [newPos, prob] in newPosDist.iteritems():
                #update
                allPossible[newPos] += self.beliefs[legalPosition]*prob
        #norm probabilities
        allPossible.normalize()
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        self.particleList = []
        for particle in range(1, self.numParticles + 1):
            index = particle % len(self.legalPositions)
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for particle in self.particleList:
                td = emissionModel[util.manhattanDistance(particle, pacmanPosition)]
                weight[particle] += td
                totalweight += td
            if totalweight == 0:
                self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.particleList[:] = [util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, x))) for x in self.particleList]
    ...
    def getBeliefDistribution(self):
        ...
        for loc in self.particleList:
            myDict[loc] += 1/ (float)(self.numParticles)
        return myDict
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        while len(self.particles) < self.numParticles:
            for roll in itertools.product(self.legalPositions, repeat = 2):
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                setGhostPos = setGhostPositions(gameState, newParticle)
                dist = getPositionDistributionForGhost(setGhostPos, i, self.ghostAgents[i])
