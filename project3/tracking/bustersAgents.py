...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        howLikelyGhostyThere = 0
        for ghosty in livingGhostPositionDistributions:
            for whereHeCouldBe, howLikelyHeThere in ghosty.iteritems():
                if howLikelyHeThere > howLikelyGhostyThere:
                    howLikelyGhostyThere = howLikelyHeThere
                    whereTheGhostAt = whereHeCouldBe
        bestAction = legal[0]
        bestActionPos = Actions.getSuccessor(pacmanPosition, bestAction)
        bestActionDistance = self.distancer.getDistance(bestActionPos, whereTheGhostAt)
        for action in legal:
            curPosition = Actions.getSuccessor(pacmanPosition, action)
            curDistance = self.distancer.getDistance(curPosition, whereTheGhostAt)
            if curDistance < bestActionDistance:
                bestActionDistance = curDistance
                bestAction = action
        return bestAction
